...
**OpenRouter API 端點（使用免費模型 `x-ai/grok-4.1-fast:free`）:**
...
8. **POST /api/ai/practice-feedback**
   ...
   - 支援 3 種 AI 模型選擇（haiku, sonnet, grok）
...
- **OpenRouter API**: 拼字檢查、例句翻譯（使用免費模型 `x-ai/grok-4.1-fast:free`）
...
4. **OpenRouter 免費模型**: 使用 `x-ai/grok-4.1-fast:free` 免費模型進行翻譯和拼字檢查
...
✅ **表單自動滾動與對焦** - 打開表單時自動滾動到輸入框並對焦，支援響應式留白（桌面 80px / 手機 20px）
